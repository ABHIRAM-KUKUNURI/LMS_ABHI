...
function addInstructor(){
   ...
   var Linked=$('#Linked').val()
        ...
        $.ajax({
                ...
                processData : false,
                data:formData,
                url:'http://127.0.0.1:9999/a/instructorUpdate',
                success:(e)=>{
...
function Show(){
    ...
    // })
    $.ajax({
        "method":"POST",contentType:"application/json",'data':JSON.stringify({email:Ins_Email}),'url':'http://127.0.0.1:9999/a/status',
      "success":function(e) {
        console.log(e)
        if (e=='Approved'){
